...
export function ExchangeWorkspaceCard({
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (!snapshot.empty) {
          ...
          const localTasks = getLocalExchangeTasks(exchange.id);
          const remoteIds = new Set(loadedTasks.map((t) => t.id));
          const combined = [
            ...loadedTasks,
          ...
          setTasks(combined);
          saveLocalExchangeTasks(exchange.id, combined);
        } else {
  ...
  async function handleAddTask(e: React.FormEvent) {
    ...
    const currentLocals = getLocalExchangeTasks(exchange.id);
    saveLocalExchangeTasks(exchange.id, [...currentLocals.filter((t) => t.id !== taskId), taskObj]);
    ...
    try {
      ...
      console.warn("Notice: Task created locally (Firestore sync deferred):", err);
    }
  }
      ...
      <div className="bg-slate-50/70 dark:bg-slate-800/80 px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-primary hover:bg-primary-hover text-white px-3 py-1.5 text-xs font-bold transition cursor-pointer shadow-xs"
          >
        ...
        <div className="p-5 space-y-3">
          ...
          {isAddingTask && (
            <form
              ...
              className="p-4 rounded-2xl border border-primary/30 bg-primary/5 dark:bg-primary/10 space-y-3 animate-in fade-in duration-200"
            >
              <div>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3.5 py-2 text-slate-900 dark:text-white outline-none focus:border-primary"
                />
              </div>
              <div className="flex flex-wrap items-center gap-2.5">
                <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                  ...
                  <select
                    ...
                    className="text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1.5 text-slate-700 dark:text-slate-200 outline-none"
                  >
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-xl bg-primary px-4 py-1.5 text-xs font-bold text-white hover:bg-primary-hover transition cursor-pointer shadow-xs disabled:opacity-50"
                  >
